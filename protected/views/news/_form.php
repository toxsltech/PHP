...
?>
                  <?php echo $form->field($model, 'title')->textInput(['maxlength' => 128]) ?>
                              ...
                              <?php /*echo  $form->field($model, 'summary')->widget ( app\components\TRichTextEditor::className (), [ 'options' => [ 'rows' => 6 ],'preset' => 'basic' ] ); //$form->field($model, 'summary')->textarea(['rows' => 6]); */ ?>
      ...
      <?= Html::submitButton($model->isNewRecord ? Yii::t('app', 'Save') : Yii::t('app', 'Update'), ['id'=> 'news-form-submit','class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
   </div>
   <?php TActiveForm::end(); ?>
</div>